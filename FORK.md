...
| Harness-neutral capability policy (definition `capability_policy` + instance `capability_policy_override`, Buzz prompt-skill sections). NO verified tool transport ships in v1: omp was downgraded to harness-managed after the HC-001 launch check proved its `--tools` flag surface does not enforce the selected set in `acp` mode (evidence + rerun criteria: `docs/hc001-omp-capability-transport.md`) | `…/managed_agents/types/capability_policy.rs`, `…/managed_agents/capability_compiler.rs` (pure compiler + save-time check), `…/managed_agents/prompt_skills.rs` (catalog/compose/byte caps), `…/managed_agents/discovery/runtime_metadata.rs` (`CapabilityTransport`, `provider_selection`), `crates/buzz-acp/src/config.rs` (`BUZZ_ACP_AGENT_ARGS_JSON` — the comma-safe transport a future verified compiler output requires) | `cargo test --manifest-path desktop/src-tauri/Cargo.toml --lib managed_agents::` — pins: `selected_tools_compile_ahead_of_subcommand` (fixture transport), `omp_tool_policy_is_rejected_harness_managed`, `unsupported_capability_error_lists_ids`, `raw_capability_flag_conflict_is_named`, `default_policy_record_hash_matches_pre_feature_baseline`, `policy_absent_persona_hash_stable_across_activation`; `cargo test -p buzz-acp` — pins: `agent_args_json_wins_over_csv_and_preserves_commas`, `malformed_agent_args_json_fails_closed`, `normalizes_omp_args_to_acp`; frontend: `cd desktop && node --import ./test-loader.mjs --experimental-strip-types --test 'src/features/agents/**/*.test.mjs'` + `pnpm exec playwright test agent-capability-policy` (after `pnpm build:e2e`) |
...
| Personal build identity (deploy branch only) | `desktop/src-tauri/tauri.conf.json` (`productName`, `identifier` with `.dev.` infix), `desktop/src-tauri/Info.plist` (`CFBundleName`, `CFBundleDisplayName` — override `productName` in menu bar/Dock), `desktop/src-tauri/src/app_state_keyring.rs` (release keychain service `buzz-desktop-<slug>`) | present only on personal deploy branches; if a merge/rebase shows them on `main` or a PR branch, the branch is wrong — drop those hunks |
