...
#[cfg(test)]
use summary::persona_drift_state;
...
pub fn spawn_agent_child(
    ...
    // Lossless JSON args transport (Phase B1): the compiler's output is
    // comma-bearing (`--tools=read,grep`), which the legacy comma-joined CSV
    // cannot carry. buzz-acp prefers the JSON var when present; CSV stays for
    // one compatibility window. Mixed-version note: a deliberately downgraded
    // buzz-acp ignores JSON and splits comma args — the bundled sidecar makes
    // this a non-default path (accepted).
