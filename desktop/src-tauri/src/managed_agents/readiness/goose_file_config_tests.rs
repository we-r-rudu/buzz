//! Goose file-config–aware requirement tests, split from `readiness.rs`
//! (file-size guard). These call `goose_requirements` directly, injecting a
//! synthetic `RuntimeFileConfig` so there is no disk I/O and tests are
//! deterministic.
